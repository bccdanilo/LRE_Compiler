...
class TimerInterrupt: public Node {
private:
	string fname;
	int ms;
	static Function *fattach;
public:
	TimerInterrupt(string _fname, int _ms) {
		this->fname = _fname;
		this->ms = _ms;
	}
	...
	virtual Value *generate(Function *func, BasicBlock *block, BasicBlock *allocblock) {
		...
		// setup arduino timer interruption function
		if (!fattach) {
			arg_types.clear();
			arg_types.push_back(fcalledtype->getPointerTo());
			arg_types.push_back(Type::getInt16Ty(global_context));
			...
			fattach = Function::Create(ftype, Function::ExternalLinkage, "timerone_attachInterrupt", mainmodule);
			fattach->setCallingConv(CallingConv::C);
		}

		vector<Value*> args;
		args.push_back(cfunc);
		args.push_back(ConstantInt::get(Type::getInt16Ty(global_context), ms));
